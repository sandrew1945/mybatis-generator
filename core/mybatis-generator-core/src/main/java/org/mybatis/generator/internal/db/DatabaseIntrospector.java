...
public class DatabaseIntrospector
{
	...
	public List<IntrospectedTable> introspectTables(TableConfiguration tc) throws SQLException
	{
		...
		removeIgnoredColumns(tc, columns); // 移除忽略的列
		calculateExtraColumnInformation(tc, columns);
		applyColumnOverrides(tc, columns); // 替换重写的数据库与Java对应关系
		calculateIdentityColumns(tc, columns); // 处理主键

		List<IntrospectedTable> introspectedTables = calculateIntrospectedTables(tc, columns); // 根据TableConfiguration生成IntrospectedTable集合
		...
		while (iter.hasNext())
		{
			// IntrospectedTable包含标的配置信息(TableConfiguration)、数据库列信息及对应Java类型
	...
	private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc, Map<ActualTableName, List<IntrospectedColumn>> columns)
	{
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet())
		{
			...
			// 生成FullyQualifiedTable
			String domainObjectName = null;
			if(!stringHasValue(tc.getDomainObjectName()))
			{
				// 如果domainObjectName没有填写，那么根据表名生成
				domainObjectName = TableUtil.getPONameByTableName(tc.getTableName());
			}
			// modi by weibin 如果domainObjectName没有填写，那么根据表名生成
...
//					tc.getProperty(PropertyRegistry.TABLE_RUNTIME_SCHEMA), tc.getProperty(PropertyRegistry.TABLE_RUNTIME_TABLE_NAME), delimitIdentifiers, context);
			FullyQualifiedTable table = new FullyQualifiedTable(stringHasValue(tc.getCatalog()) ? atn.getCatalog() : null, stringHasValue(tc.getSchema()) ? atn.getSchema() : null, atn.getTableName(),
					domainObjectName, tc.getAlias(), isTrue(tc.getProperty(PropertyRegistry.TABLE_IGNORE_QUALIFIERS_AT_RUNTIME)), tc.getProperty(PropertyRegistry.TABLE_RUNTIME_CATALOG),
					tc.getProperty(PropertyRegistry.TABLE_RUNTIME_SCHEMA), tc.getProperty(PropertyRegistry.TABLE_RUNTIME_TABLE_NAME), delimitIdentifiers, context);
