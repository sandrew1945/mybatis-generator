...
public class Context extends PropertyHolder
{
	 ...
	 * 
	 * @param callback 执行过程回调，如果不需要执行过程，可以传null
	 * @param warnings 警告信息集合
	...
	public void introspectTables(ProgressCallback callback, List<String> warnings, Set<String> fullyQualifiedTableNames) throws SQLException, InterruptedException
	{
		...
		// JavaTypeResolver默认实现为JavaTypeResolverDefaultImpl，用来转换数据库类型与Java类型的关系，对象中包含context信息
		...
		try
		{
			...
			{
				// 获取完整的数据库表，包括Catalog，Schema及表名，分隔符为"."
				String tableName = composeFullyQualifiedTableName(tc.getCatalog(), tc.getSchema(), tc.getTableName(), '.');
				// 判断表名是否在fullyQualifiedTableNames中，如果fullyQualifiedTableNames为空，则没有此验证，生成全部配置的表
				...
				// 获取需要生成的表的相关信息(IntrospectedTable),IntrospectedTable包含标的配置信息(TableConfiguration)、数据库列信息及对应Java类型
	...
	public void generateFiles(ProgressCallback callback, List<GeneratedJavaFile> generatedJavaFiles, List<GeneratedXmlFile> generatedXmlFiles, List<String> warnings) throws InterruptedException
	{
		...
		if (introspectedTables != null)
		{
			for (IntrospectedTable introspectedTable : introspectedTables)
			{
				...
				introspectedTable.initialize();	 // 初始化
				introspectedTable.calculateGenerators(warnings, callback);	// 计算生成的model、mapper接口、mapper XML
