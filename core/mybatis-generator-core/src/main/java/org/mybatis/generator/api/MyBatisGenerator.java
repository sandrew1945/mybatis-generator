...
public class MyBatisGenerator
{
	 ...
	 * method can be cancelled through the ProgressCallback interface.
	 *
	 * @param callback 执行过程回调接口，如果不需要过程信息，可以传null
	 * @param contextIds contextID的集合，可以为空，如果为空，则使用配置文件解析出的context集合
	 * @param fullyQualifiedTableNames 要生成的表明，如果为null，那么生成全部配置的表，例如：表名= "foo" and schema = "bar", then the fully qualified table name is "foo.bar".
	 * @param writeFiles 如果为TRUE，那么生成的文件会被写入磁盘，否则不写入
	 * @throws SQLException
	 *             the SQL exception
	 * @throws IOException
	 *             Signals that an I/O exception has occurred.
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames, boolean writeFiles) throws SQLException, IOException, InterruptedException
	{
		...
		// calculate the contexts to run 要执行的contexts集合，context为配置文件中的context节点
		...
		// setup custom classloader if required 加载配置的数据库驱动
		...
		{
			// 获取每个context的步骤，连接数据库(1次)  + 解析需要生成的表数量
		...
		for (Context context : contextsToRun)
		{
			// 解析每个context,并解析TableConfiguration，生成IntrospectedTable集合，
			// IntrospectedTable包含表的配置信息(TableConfiguration)、数据库列信息及对应Java类型
			// 该集合存到对应的context对象中
		...
		for (Context context : contextsToRun)
		{
			// 获取每个context的步骤，生成model(1) + 生成mapper接口(1) + 生成mapper xml(如果不为空则为1)
			totalSteps += context.getGenerationSteps(); // 3
		}
